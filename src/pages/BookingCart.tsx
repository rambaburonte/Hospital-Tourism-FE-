...
//     if (paymentSuccess) {
//       try {
//         // Send payment status confirmation to backend
//         await axios.put(`${BASE_URL}/api/AddToCart/bookings/update-payment-status`, selectedItems, {
//           headers: { 'Content-Type': 'application/json' }
//         });
//         console.log('Payment status updated successfully in backend.');
//       } catch (apiError) {
//         console.error('Error updating payment status in backend:', apiError);
//         // Decide how to handle this error - maybe show a warning but proceed with frontend update
//       }
...
//     try {
//       // Delete multiple items using the bulk delete API
//       await axios.delete(`${BASE_URL}/api/AddToCart/delete-multiple`, {
//         data: selectedItems // Modified to send the array directly
//       });
  ...
  useEffect(() => {
    ...
    const fetchBookings = async () => {
      ...
      try {
        const response = await axios.get(`${BASE_URL}/api/AddToCart/user/${user.id}`);
        const bookings = response.data
          .filter((item: any) => item.serviceTypes.toLowerCase() !== 'pharmacy')
          .map((item: any) => ({
            bookingId: item.bookingId,
            bookingStatus: item.bookingStatus,
            bookingAmount: item.bookingAmount,
