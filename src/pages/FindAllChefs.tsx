...
const ChefsList: React.FC = () => {
  ...
  return (
    <section className="min-h-screen bg-[#f7f8fa] py-14 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10">
          <h1 className="text-4xl font-bold text-gray-900">Discover Professional Chefs</h1>
          <p className="text-gray-500 mt-2">Find the perfect chef by location, cuisine, and rating</p>
        </div>

        <div className="bg-white/50 backdrop-blur-lg border border-white/30 rounded-2xl shadow-xl p-6 mb-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Select value={locationFilters.city} onValueChange={(value) => handleLocationFilterChange('city', value)}>
              <SelectTrigger><SelectValue placeholder="City" /></SelectTrigger>
              <SelectContent>
                {uniqueValues.cities.map((city) => <SelectItem key={city} value={city}>{city}</SelectItem>)}
              </SelectContent>
            </Select>

            <Select value={locationFilters.state} onValueChange={(value) => handleLocationFilterChange('state', value)}>
              <SelectTrigger><SelectValue placeholder="State" /></SelectTrigger>
              <SelectContent>
                {uniqueValues.states.map((state) => <SelectItem key={state} value={state}>{state}</SelectItem>)}
              </SelectContent>
            </Select>

            <Select value={locationFilters.country} onValueChange={(value) => handleLocationFilterChange('country', value)}>
              <SelectTrigger><SelectValue placeholder="Country" /></SelectTrigger>
              <SelectContent>
                {uniqueValues.countries.map((country) => <SelectItem key={country} value={country}>{country}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="mt-6 flex flex-wrap justify-between items-center gap-4">
            <Select value={styleFilter || 'all'} onValueChange={(value) => setStyleFilter(value === 'all' ? '' : value)}>
              ...
              <SelectContent>
                <SelectItem value="all">All Styles</SelectItem>
                {uniqueValues.styles.map((style) => <SelectItem key={style} value={style}>{style}</SelectItem>)}
              </SelectContent>
            </Select>
            <button
              onClick={clearFilters}
              className="px-4 py-2 text-sm font-medium text-gray-600 bg-white border rounded-md hover:bg-gray-100"
              disabled={!searchQuery && !styleFilter && !locationFilters.city && !locationFilters.state && !locationFilters.country}
            >
              Clear Filters
            </button>
          </div>
        ...
        {filteredChefs.length === 0 ? (
          <div className="text-center text-gray-600">No chefs match your search.</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredChefs.map((chef) => (
              <div key={chef.chefID} className="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition duration-300">
                <img src={chef.chefImage} alt={chef.chefName} className="w-full h-64 object-cover" onError={(e: any) => e.target.src = 'https://via.placeholder.com/300'} />
                <div className="p-5">
                  <h2 className="text-xl font-bold text-gray-800 mb-2">{chef.chefName}</h2>
                  <p className="text-gray-500 mb-3 line-clamp-2">{chef.chefDescription}</p>
                  <div className="text-sm text-gray-600 space-y-1">
                    <p><strong>Location:</strong> {chef.location.city}, {chef.location.state}, {chef.location.country}</p>
                    <p><strong>Experience:</strong> {chef.experience}</p>
                    <p><strong>Styles:</strong> {chef.styles.join(', ')}</p>
                    <p><strong>Rating:</strong> {chef.chefRating} ⭐</p>
                    <p className="text-green-600 font-semibold mt-2">From ${chef.price}</p>
                  </div>
                  <button
                    onClick={() => handleBookClick(chef.chefID)}
