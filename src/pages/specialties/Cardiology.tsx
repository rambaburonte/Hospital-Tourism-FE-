...
interface Hospital {
  id: string;
  name: string;
  specialties: string[]; // Assuming specialties is an array of strings
  location: string;
  contact: string;
...
const Cardiology: React.FC = () => {
  ...
  useEffect(() => {
    const fetchHospitals = async () => {
      try {
        const response = await fetch('http://localhost:4545/api/hospitals/getall/hospitals/active');
        if (!response.ok) {
          throw new Error('Failed to fetch hospitals');
        }
        const data: Hospital[] = await response.json();
        
        // Filter hospitals with cardiology in their specialties
        const cardiologyHospitals = data.filter(hospital =>
          hospital.specialties?.includes('Cardiology')
        );
        
        // Optional: Sort hospitals by some cardiology-related metric (e.g., name or ranking)
        const sortedHospitals = cardiologyHospitals.sort((a, b) =>
          a.name.localeCompare(b.name) // Example: Sort alphabetically by name
        );
  ...
  return (
    <div className="specialty-page min-h-screen bg-gray-50 pt-20">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <h1 className="text-5xl font-extrabold text-gray-900 mb-6">Cardiology</h1>
        <p className="text-xl text-gray-700 mb-8 max-w-3xl leading-relaxed">
          Our Cardiology Department is committed to delivering world-class heart care. Below is a list of our active hospitals offering cardiology services.
        </p>
        ...
        <section className="mb-12">
          ...
          {loading ? (
            ...
            <p className="text-gray-600">No hospitals with cardiology services found.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {hospitals.map(hospital => (
                <div key={hospital.id} className="bg-white p-6 rounded-lg shadow-md">
                  ...
                  <p className="text-gray-600 mb-1"><strong>Contact:</strong> {hospital.contact}</p>
        ...
        {/* Add other sections (Cardiology Services, Common Heart Conditions, etc.) as needed */}
      </div>
    </div>
  );
};
