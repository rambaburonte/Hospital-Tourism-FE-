...
const UpdateChef: React.FC = () => {
  ...
  useEffect(() => {
    const fetchChef = async () => {
      try {
        const response = await fetch(`https://healthtourism-5.onrender.com/api/chefs/chef-By/Id/${chefID}`);
        if (!response.ok) {
  ...
  const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      const response = await fetch(`https://healthtourism-5.onrender.com/api/chefs/${chefID}`, {
        method: 'PUT',
