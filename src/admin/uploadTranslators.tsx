...
const UploadTranslators: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedLocation || !name || !description || !image || !rating || !languages) {
      setMessage('All fields are required');
      return;
    }

    try {
      const res = await axios.post('http://localhost:8080/api/translators/add', {
        translatorName: name,
        translatorDescription: description,
        translatorImage: image,
      ...
      if (res.status === 200 || res.status === 201) {
        ...
        setMessage('Failed to upload translator');
      }
    } catch (err) {
      console.error(err);
