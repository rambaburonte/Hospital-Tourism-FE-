...
const Users: React.FC = () => {
  ...
  const { data, isLoading, error, isFetching } = useQuery<{ data: User[]; total: number }, Error>({
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${theme === 'dark' ? 'bg-slate-900 text-slate-50' : 'bg-slate-50 text-slate-900'}`}>
      <AdminSidebar />
                    ...
                    {attachmentTypes.map(({ key, icon: Icon, title }) => {
                      ...
                      return url ? (
                        <Button
                          ...
                          size="sm"
                          className={`flex items-center space-x-1 text-xs ${!url ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-200 dark:hover:bg-slate-700'}`}
                          onClick={() => handleSwitchAttachment(url)}
                          disabled={!url}
