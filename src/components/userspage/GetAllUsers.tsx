...
// // Updated User interface based on API response
// interface User {
//   id: number;
//   name: string | null;
//   email: string;
...
//   patientreportsUrl: string | null;
//   address: string | null;
//   bookingIds: string | null; // Added based on response
// }
...
//   // Fallback for array response (current API behavior)
//   if (Array.isArray(responseData)) {
//     console.warn(
...
// // Rest of the component remains the same as provided
// const Users: React.FC = () => {
//   const { theme, search, sort, isAdmin, toggleTheme, setSearch, setSort } = useAppStore();
//   const [page, setPage] = useState(1);
//   const [previewUrl, setPreviewUrl] = useState<string | null>(null);
...
//       }>
//         <Dialog open={!!previewUrl} onOpenChange={(isOpen) => { if (!isOpen) { setPreviewUrl(null); setPreviewError(null); } }}>
//           <DialogContent className="max-w-2xl w-[90vw] md:max-w-3xl lg:max-w-4xl xl:max-w-5 my-4 bg-white dark:bg-slate-800 p-0 shadow-xl rounded-lg">
//             <DialogHeader className="p-4 sm:p-6 border-b dark:border-slate-700">
//               <DialogTitle className="text-xl font-semibold">File Preview</DialogTitle>
//               <DialogDescription className="text-sm text-slate-500 dark:text-slate-400">
//                 Viewing: {previewUrl ? previewUrl.substring(previewUrl.lastIndexOf('/') + 1) : 'File'}
...
//             </div>
//             <DialogFooter className="p-4 sm:p-6 border-t dark:border-slate-700 flex flex-col sm:flex-row sm:justify-end space-y-2 sm:space-y-0 sm:space-x-2">
//               {isAdmin && previewUrl && !previewError && (previewType === 'image' || previewType === 'pdf' || !previewType) && (
//                 <Button onClick={handleDownloadFromModal}>
//                   <Download className="h-4 w-4 mr-2" />
//                   Download File
//                 </Button>
...
//                               size="icon"
//                               className="h-7 w-7 text-slate-600 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400"
//                               onClick={() => handleLinkClick(item.url!, user)} // Pass user object
//                               title={`View ${item.title}`}
//                             >
//                               <item.icon className="h-4 w-4" />
//                             </Button>
...
//         </div>
//       }>
//         <Dialog open={!!previewUrl} onOpenChange={(isOpen) => { if (!isOpen) { setPreviewUrl(null); setPreviewError(null); setSelectedUser(null); } }}>
...
//               <DialogTitle className="text-xl font-semibold">File Preview</DialogTitle>
//               <DialogDescription className="text-sm text-slate-500 dark:text-slate-400">
//                 Viewing: {previewUrl ? previewUrl.substring(previewUrl.lastIndexOf('/') + 1) : 'File'} {selectedUser ? `for ${selectedUser.name || selectedUser.email}` : ''}
//               </DialogDescription>
//             </DialogHeader>
...
//                         disabled={!url}
//                         title={url ? `View ${title}` : `${title} not available`}
//                       >
...
//             <div className="p-1 sm:p-4 h-[50vh] md:h-[55vh] lg:h-[60vh] overflow-y-auto bg-slate-50 dark:bg-slate-800/50">
//               {previewError && (
//                 <div className="flex flex-col items-center justify-center h-full text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 p-4 rounded-md">
//                   <AlertCircle className="w-10 h-10 mb-3" />
//                   <p className="text-lg font-semibold">Preview Error</p>
...
//                 </Button>
//               )}
//               <Button variant="outline" onClick={() => { setPreviewUrl(null); setPreviewError(null); setSelectedUser(null); }}>
//                 Close
//               </Button>
//             </DialogFooter>
//           </DialogContent>
