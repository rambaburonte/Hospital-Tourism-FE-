...
//     setIsLoading(true);
//     try {
//       const response = await axios.post(`${BASE_URL}/sub/admin/subadminregister`, {
...
//         }, {} as Record<string, boolean>)
//       );
//       setTimeout(() => navigate('/subadmin/dashboard'), 1500);    } catch (error) {
...
const SubAdminRegister = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => navigate('/subadmin/dashboard'), 1500);
    } catch (error) {
      const errorMsg = error?.response?.data?.error || 'Registration failed. Please try again.';
      toast.error(errorMsg);
    } finally {
